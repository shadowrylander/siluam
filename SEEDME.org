...
*  ইয়ো, ইয়ো, ইয়ো, চলো যাই, যাও, যাও! Yo, yo, yo, let's go, go, go!

This holds the ~early-init~ and ~init~ files for my version of a literate configuration for
...
#+begin_src emacs-lisp :noweb-ref no :tangle early-init.el :exports none
<<early-init.el>>
#+end_src
...
#+begin_src emacs-lisp :noweb-ref no :tangle init.el :exports none
<<init.el>>
#+end_src
